...
public class UIAtlasMaker : EditorWindow
{
    ...
    List<Texture> GetSelectedTextures()
    {
        ...
        if (Selection.objects != null && Selection.objects.Length > 0)
        {
            ...
            foreach (Object o in objects)
            {
                ...
                //Taesang CompressAtlas
                if (NGUISettings.atlas != null && NGUISettings.atlas.name + "_Alpha" == tex.name) continue;
                //Taesang End
    ...
    static public List<SpriteEntry> CreateSprites(List<Texture> textures)
    {
        ...
        foreach (Texture tex in textures)
        {
            ...
            if (newWidth > 0 && newHeight > 0)
            {
                ...
                //Taesang remove edge
                sprite.removeEdge = NGUISettings.removeEdge ? 1 : 0;
                //End Taesang
    ...
    void OnGUI()
    {
        ...
        //Taesang remove edge
        GUILayout.BeginHorizontal();
        NGUISettings.removeEdge = EditorGUILayout.Toggle("Remove Edge", NGUISettings.removeEdge, GUILayout.Width(100f));
        GUILayout.Label("Remove Edge");
        GUILayout.EndHorizontal();
        //End Taesang
            ...
            if (mat != null)
            {
                ...
                if (shader != null)
                {
                    if (shader.name == "Unlit/Transparent Colored")
                    {
                        ...
                        fixedShader = true;
                        //Taesang CompressAtlas
                        NGUISettings.compressAtlas = false;
                        //Taesang End
                    }
                    ...
                    //Taesang CompressAtlas
                    else if (shader.name == "Unlit/Transparent Colored Compress")
                    {
                        NGUISettings.atlasPMA = false;
                        fixedShader = true;
                        NGUISettings.compressAtlas = true;
                    }
                    else if (shader.name == "Unlit/Premultiplied Colored Compress")
                    {
                        NGUISettings.atlasPMA = true;
                        fixedShader = true;
                        NGUISettings.compressAtlas = true;
                    ...
                    //Taesang End
                    else if (shader.name == "RPG/UI/Transparent Gray Scale")
                    {
                        NGUISettings.atlasPMA = false;
                        fixedShader = true;NGUISettings.compressAtlas = false;
                    }
                }
            }
        }
        ...
        //Taesang CompressAtlas
        GUILayout.BeginHorizontal();
        NGUISettings.compressAtlas = EditorGUILayout.Toggle("Compress", NGUISettings.compressAtlas, GUILayout.Width(100f));
        GUILayout.Label("if on, atlas texture compress to etc1");
        GUILayout.EndHorizontal();
        ...
        if (NGUISettings.atlas != null)
        {
            ...
            if (mat != null)
            {
                ...
                if (shader != null)
                {
                    if (NGUISettings.compressAtlas == true)
                    {
                        if (shader.name == "Unlit/Transparent Colored")
                        {
                            mat.shader = Shader.Find("Unlit/Transparent Colored Compress");
                            CreateAlphaTexture();
                        ...
                        {
                            mat.shader = Shader.Find("Unlit/Premultiplied Colored Compress");
                            CreateAlphaTexture();
                        ...
                        {
                            mat.shader = Shader.Find("RPG/UI/Transparent Gray Scale Compress");
                            CreateAlphaTexture();
                    ...
                    {
                        if (shader.name == "Unlit/Transparent Colored Compress")
                        {
                            mat.shader = Shader.Find("Unlit/Transparent Colored");
                            mat.SetTexture("Alpha Tex", null);
                            RemoveAlphaTexture();
                            RevertMainTextureSetting();
                            AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport);
                        ...
                        {
                            mat.shader = Shader.Find("Unlit/Premultiplied Colored");
                            mat.SetTexture("Alpha Tex", null);
                            RemoveAlphaTexture();
                            RevertMainTextureSetting();
                            AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport);
                        ...
                        else if (shader.name == "RPG/UI/Transparent Gray Scale Compress")
                        {
                            mat.shader = Shader.Find("RPG/UI/Transparent Gray Scale");
                            mat.SetTexture("Alpha Tex", null);
                            RemoveAlphaTexture();
                            RevertMainTextureSetting();
                            AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport);
            ...
            if (create)
            {
                ...
                if (!string.IsNullOrEmpty(path))
                {
                    ...
                    if (mat == null)
                    {
                        ...
                        if (NGUISettings.compressAtlas)
                        {
                            shader = Shader.Find(NGUISettings.atlasPMA ? "Unlit/Premultiplied Colored Compress" : "Unlit/Transparent Colored Compress");
                        }
                        else
                        {
                            shader = Shader.Find(NGUISettings.atlasPMA ? "Unlit/Premultiplied Colored" : "Unlit/Transparent Colored");
                        }
                        //Taesang Original Code
        ...
        // 2016년 5월 17일 
        // 김세훈 
        // Atlas 이미지 분리 Png로 저장 
        // 저장 경로 Asset/아틀라스 이름
        GUILayout.Space(5f);
        if (GUILayout.Button("Disassemble Atlas"))
        {
            DisassembleAtlas();
        }
    }
    ...
    void CreateAlphaTexture()
    {
        ...
        string texPath = NGUIEditorTools.GetSaveableTexturePath(NGUISettings.atlas);
        Texture2D tex = NGUIEditorTools.ImportTexture(texPath, true, false, false);
        ...
        for (int y = 0; y < height; ++y)
        {
            ...
            {
                int index = y * width + x;
                newPixels[index].r = pixels[index].a;
                newPixels[index].g = pixels[index].a;
                newPixels[index].b = pixels[index].a;
                newPixels[index].a = pixels[index].a;
            }

            ShowProgress((float)y / height);
        }

        Texture2D alphaTex = new Texture2D(width, height, TextureFormat.RGBA32, false);
        alphaTex.SetPixels32(newPixels);
        alphaTex.Apply();
        string path = NGUIEditorTools.GetSaveableAlphaTexturePath(NGUISettings.atlas);
        ...
        TextureImporter importerMain = AssetImporter.GetAtPath(texPath) as TextureImporter;
        ...
        //importer.SetPlatformTextureSettings("Android", 2048, TextureImporterFormat.ETC_RGB4, 100, false);
        //importer.SetPlatformTextureSettings("iPhone", 2048, TextureImporterFormat.PVRTC_RGB4, 100, false);
        importer.SetPlatformTextureSettings("Android", 2048, TextureImporterFormat.RGBA32, 100, false);
        importer.SetPlatformTextureSettings("iPhone", 2048, TextureImporterFormat.RGBA32, 100, false);

        AssetDatabase.ImportAsset(path);
        AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport);

        alphaTex = NGUIEditorTools.ImportTexture(path, false, false, false);
    ...
    void RemoveAlphaTexture()
    {
        //string path = NGUIEditorTools.GetSaveableAlphaTexturePath(NGUISettings.atlas);
        //if (!string.IsNullOrEmpty(path)) AssetDatabase.DeleteAsset(path);
    }

    void RevertMainTextureSetting()
    {
        string texPath = NGUIEditorTools.GetSaveableTexturePath(NGUISettings.atlas);
        TextureImporter importerMain = AssetImporter.GetAtPath(texPath) as TextureImporter;
        importerMain.textureType = TextureImporterType.Default;
    ...
    void RefreshSprites()
    {
        List<UISprite> sprites = NGUIEditorTools.FindAll<UISprite>();

        for (int i = 0; i < sprites.Count; ++i)
        {
            UISprite sp = sprites[i];

            if (sp.atlas == NGUISettings.atlas)
            {
    ...
    // 김세훈 
    // 아틀라스 이미지 분해
    private string m_saveFolderPath = "C:\\DisassembleAtlas";
    private void DisassembleAtlas()
    {
        if (NGUISettings.atlas == null)
        {
            EditorUtility.DisplayDialog("Error", "Atlas Null", "OK");
            return;
        }
        ...
        try
        {
            ...
            foreach (var spriteData in NGUISettings.atlas.spriteList)
            {
                SavePng(tex, spriteData);
            }

            SetTextureImporterFormat(tex, false);
            EditorUtility.DisplayDialog("Success", m_saveFolderPath + " / " + NGUISettings.atlas.name + "를 확인해 주세요", "OK");
        }
        catch
        {
            EditorUtility.DisplayDialog("Error", "Fail", "OK");
        }
    }

    private void SavePng(Texture2D tex, UISpriteData spriteData)
    {
        // png 생성
        byte[] imageByte;
        ...
        int y = NGUISettings.atlas.texture.height - (spriteData.y + height);
        if (y < 0) y = 0;

        var part = tex.GetPixels(spriteData.x, y, width, height);
        var newtex = new Texture2D(width, height, TextureFormat.ARGB32, true);
        newtex.SetPixels(part);
        newtex.Apply();
        ...
        {
            if(i == 0)
                filePath = m_saveFolderPath + "/" + NGUISettings.atlas.name + "/" + spriteData.name + ".png";
            else
                filePath = m_saveFolderPath + "/" + NGUISettings.atlas.name + "/" + spriteData.name + "(" + i + ")" + ".png";    
            i++;
    ...
    void DeleteAllFile()
    {
        if (Directory.Exists(m_saveFolderPath) == false)
            Directory.CreateDirectory(m_saveFolderPath);
        // 폴더내 파일 삭제
        var folderPath = m_saveFolderPath + "/" + NGUISettings.atlas.name;
