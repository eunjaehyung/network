...
public static class NGUIEditorTools
{
	...
	static public string GetSaveableAlphaTexturePath(UIAtlas atlas)
	{
		string path = "";
		path = AssetDatabase.GetAssetPath(atlas.GetInstanceID());
		path = string.IsNullOrEmpty(path) ? "Assets/" + atlas.name + "_Alpha.png" : path.Replace(".prefab", "_Alpha.png");
		return path;
	}
	//Taesang End
